...
| **5** — the live view swaps | **In progress.** The orthographic swap is in: `Scene3DCanvas` replaces `SceneryCanvas`, `applyLiveCam` reproduces the map transform (pixel-aligned by test), shadows refit per camera move, DOM overlay untouched. Remaining: perspective and the follow camera once cars, crews and signs are in-scene |
