...
- **`feat/sim-2d-overhaul`: the 3D port is COMPLETE (2026-08-01).** All 6 increments of `3d-port-plan.md` landed; one three.js scene draws everything (world, ink decals, lofted cars, garage boards, pit crews and furniture, orbit camera, moods/night). `SceneryCanvas`/`drawScene` are deleted. The 2D frame-rate campaign stays REVERTED: **never re-add a mitigation** (detail ladder, cull discs, caches, batching, node budget, frame cap, baked bitmap, perf lab). Probes: `npx tsx scripts/scene3d-preview.ts` (the live scene), `npx tsx scripts/scenery-preview.ts` (2D op-list reference, probe-only).
