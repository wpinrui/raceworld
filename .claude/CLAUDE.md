...
- **`feat/sim-2d-overhaul` renderer, stripped and now porting to real 3D (2026-07-31).** The whole frame-rate campaign is REVERTED: no detail ladder, no cull discs, no geometry/Path2D/paint caches, no group batching, no warmed swap, no node budget, no frame cap, no baked bitmap, no perf lab, no SVG world layer. One canvas draws `sceneryScene`'s op list at full fidelity, always, and keeps shipping until the 3D view swaps in. **Do not re-add a mitigation**: if it is slow, that is an argument for finishing the 3D port, planned increment by increment in `3d-port-plan.md`. Probes: `npx tsx scripts/scenery-preview.ts` (2D reference), `npx tsx scripts/scene3d-preview.ts` (the 3D scene).
