...
- **`feat/sim-2d-overhaul` renderer, stripped for the WebGL port (2026-07-31).** The whole frame-rate campaign is REVERTED: no detail ladder, no cull discs, no geometry/Path2D/paint caches, no group batching, no warmed swap, no node budget, no frame cap, no baked bitmap, no perf lab, no SVG world layer. One canvas draws `sceneryScene`'s op list at full fidelity, always. Both tarmac surfaces carry their driven-in ink again (`track-surface.ts`, `pit-surface.ts`). **Do not re-add a mitigation**: if it is slow, that is an argument for finishing the WebGL port. Probe: `npx tsx scripts/scenery-preview.ts`, which builds the scene through the same calls the map does.
