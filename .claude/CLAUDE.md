...
- **`feat/sim-2d-overhaul` renderer perf.** Canvas beats SVG (58 fps / 16 long frames vs 43 / 140). Residual pit-straight raster dips are ACCEPTED. His machine blits big canvases slowly: never reintroduce full-frame `drawImage` without benchmark proof. Probes: the 'n' key **perf lab** behind `DEBUG_KEYS` (scripted camera shots, fixed frame counts, ablates the MITIGATIONS in `src/lib/ui/perf-flags.ts` as well as the layers, results in a modal with a copy button and 1% lows; it replaced the lap-aligned bench, which needed a live race and only ablated layers), `scripts/canvas-cost-check.ts`, `npm run zoom:check`.
