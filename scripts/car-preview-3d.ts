...
async function main() {
  ...
  writeFileSync(page, '<!doctype html><html><head><meta charset="utf-8"><title>car 3d</title>'
    ...
    + '#bar button{background:#1A1F27;color:#FFFFFF;border:1px solid #2A313C;border-radius:4px;'
    + 'padding:4px 10px;font:inherit;cursor:pointer;height:28px}'
    + '#bar button.on{border-color:#00D9FF;color:#00D9FF}'
    + '#hud{position:fixed;top:46px;left:10px;z-index:1;padding:10px 12px;border-radius:6px;'
    + 'background:rgba(10,12,16,.85);font:12px ui-monospace,monospace;color:#FFFFFF;'
    ...
    + '<label>Cars <select id="fleet">'
    + '<option value="1">1</option><option value="30" selected>30</option>'
    + '<option value="60">60</option></select></label>'
    + '<label>Lock <select id="lock">'
    + '<option value="-1" selected>auto</option><option value="0">L0</option>'
    + '<option value="1">L1</option><option value="2">L2</option>'
    + '<option value="3">L3</option><option value="4">L4</option></select></label>'
    + '<button id="tint">Tier tint</button>'
    + '<label>Zoom <input id="zoom" type="range" min="0" max="1000" step="1" value="1000"></label>'
    + '<span id="zoomv">&nbsp;</span>'
    + '</div><div id="hud"></div><canvas id="gl"></canvas>'
