...
Note that drivers have 5 stats: pace, wet-weather pace, overtaking, smoothness and consistency. All of them are out of 100. The overall stat (just for user-friendliness) is calculated by taking 0.4\*pace + 0.26\*consistency + 0.1\*overtaking + 0.03\*wet weather pace + 0.21\*smoothness. These weights are empirically calibrated (each ∝ the attribute's measured marginal effect on race results, with the car equalised and technical DNFs removed, one attribute varied at a time); pace and wet-weather pace split the measured speed impact by weather exposure (effective wet fraction ≈ 0.075). This is just a cosmetic overall that the player can use to sort, however it is used (on the internal true ratings) to see if the driver has hit his potential (which means no further improvement can happen, and the driver can only plateau until prime end)
...
Pit strategy (imperfect information). A perfect-information optimiser exists as a benchmark (the god-mode "Perfect strategy" panel, run on the race's TRUE tyre data). The racing AI instead plans on a per-team BELIEF that starts as an educated guess and sharpens as its cars run. Two things are hidden from the pit wall:
...
- Live tyre CONDITION is read only in coarse 15-point buckets (0-15, …, 75-90, 90-100), so the team can't read exact wear. It INFERS the wear rate from how the bucketed condition has moved this stint, which sharpens with laps — and there is no signal until condition leaves the fresh (top) bucket, so a long opening stint with no bucket crossing leaves the team uncertain and therefore conservative.

Both garages feed ONE team belief (pooled): each observation is normalised by that driver's own (known) smoothness to recover the driver-independent base rate, then re-applied per driver via smoothness. Guess quality is flat across teams (no skill tiers) — just randomness.

The optimiser is re-solved every lap (so it adapts to weather and to learned wear), searches all five compounds, and includes the projected moisture penalty from the team's FALLIBLE forecast — so it pits proactively for inters/wets at the crossover and can be wrong-footed by phantom/unforeseen rain. It outputs a pit WINDOW whose width is the team's current wear uncertainty (wide early, tight once learned), capped before the believed cliff.

Within the window the car chooses which lap to actually box:
- Forced: on a badly wrong tyre for the real conditions (e.g. slicks in a downpour), or the real tyre about to fall off the cliff, or the window's last lap — box regardless of traffic.
- Voluntary stops require CLEAR AIR on rejoin (no point fitting fresh tyres to sit in dirty air). Within clear air, an undercut chance (a beatable car directly ahead) lets it pit before the believed-optimal lap; otherwise it waits until at/after that lap. Overcut is not modelled (no tyre warmup/temperatures).
