...
This drives grounded pieces (every figure an in-game aggregate stat; texture is limited to details the sim does not model, so it can never be contradicted):
...
- **Championship arc** — sparse standalone pieces firing only at real inflections (a lead eroding or extending, a decider, a lead change), distinguishing a comeback earned on merit from one handed over by the leader's retirements. Runs for both the drivers' and the constructors' titles.
...
- **Expectation checkpoints** — a couple of times a season, who is running above or below their preseason billing (drivers and teams) — "who's cooking vs trash", measured against projection, not the last race.
- **Season review + archetype retrospectives** — pays off the preview: how the title was won, framed by the full championship-battle taxonomy (wire-to-wire, comeback on merit vs handed back by the early leader's retirements, three-way fight, title-on-podiums-without-a-win, a defining win streak, outright domination, or a late decider), who beat or missed their projection, the best of the rest; plus the season's individual stories from an **archetype classifier** (an overachiever dragging a lesser car to podiums, a preseason pick who flopped, a rookie beating a veteran team-mate, a late-career resurgence), teammate-battle verdicts (one driver routing the other on equal machinery), and cross-team duels (a parallel fight among the fast cars, or a midfield battle, settled over the year). Tier coverage (#90) extends this below the front: the best-of-the-rest fight (a compressed midfield band, a surge from a projected backmarker, or a clear midfield win) and the backmarker tier (a new team's tough debut, a tail-ender scoring against the odds, or a tight last-place battle). The classifier tags the season's shapes and frames coverage rather than spawning an article per archetype; wet/safety-car-dependent archetypes are deferred until that data is persisted.

Quality over quantity (#92): low-signal producers — the transfer rumour mill, the point-in-time title-fight and title-scenario pieces, the generic mid-season feature, and the old per-round opinion column — are pruned in favour of these higher-signal, arc-aware pieces. The factual clinch/lead-change announcement stays. Article copy is authored to the project's hand-written newsroom voice.
...
Note that drivers have 5 stats: pace, wet-weather pace, overtaking, smoothness and consistency. All of them are out of 100. The overall stat (just for user-friendliness) is calculated by taking 0.44\*pace + 0.232\*consistency + 0.097\*overtaking + 0.025\*wet weather pace + 0.206\*smoothness. These weights are empirically calibrated (re-baselined for #102): each ∝ the attribute's measured marginal effect on race results, with the car equalised, form neutralised and technical DNFs removed, one attribute varied at a time (`npm run overall:calibrate`). Pace and wet-weather pace split the measured speed impact by weather exposure, and since the wet rating counts double in the wet (#102), wet's share of speed is 2E[m]/(1+E[m]) ≈ 0.054, with E[m] ≈ 0.028 (the lap-weighted mean moisture from the live model). This is just a cosmetic overall that the player can use to sort, however it is used (on the internal true ratings) to see if the driver has hit his potential (which means no further improvement can happen, and the driver can only plateau until prime end)

# Simulation engine
For a given lap, the simulation works like this:
A car with 75 pace will take exactly 100 seconds.
This is modified by driver pace, where 75 means no effect, every +5 driver pace maps to a 0.15 second bonus, every -5 maps to a 0.15 second penalty, varied linearly (0.03s per point). The wet-weather rating works the same way but counts double in the wet (see below).
...
Wet-weather ability: pace and wet-weather ability blend by moisture around the 75 baseline, but the WET rating counts DOUBLE in the wet (#102). The driver delta to lap time is (1 - moisture)*(pace - 75) + 2*moisture*(wetWeather - 75), scaled at 0.03s per point: dry, a pace point is worth 0.03s/lap; at full wet, a wet-rating point is worth 0.06s/lap (and pace's influence fades to zero).
