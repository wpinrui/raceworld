...
| `legends` | legends (#93) | **"Remember this driver?", three a year on a fixed 4-month grid (any season phase).** One retired driver per slot, picked deterministically (seeded, never repeating across a save) from everyone retired at least `RETIREMENT_SEASONS_OUT` (5) years, i.e. `lastRacedYear <= year - 5`. An adaptive mini-bio: career line, then a moment of brilliance (winners) OR the ousting story (teammate head-to-head + who took the seat, for non-winners), an optional near-miss beat (skipped for multiple champions), the era's rivals (teammates / title rivals / peers, each from their own record), and a placement on the all-time spectrum (GOAT / loved / footnote). Facts are assembled SERVER-SIDE from the archive (`legends-data.ts`, `buildLegendData`) and passed on `ctx.legends`; the producer only renders prose, with gendered pronouns resolved from history data + a live-captured `driver_genders` table. Empty until a save has a driver gone five years. Carries an `absoluteDate` (its grid slot), bypassing the round-based `articleDate`. Interrupts by default. |
