...
| `championship_state` | titleScenario | **Forward-looking preview, race week.** "How X can clinch the title at the next race": the exact permutations (worst finish that still clinches, the points swing, win/lose-ground scenarios) for the drivers' and constructors' titles independently, plus the two finale deciders. Drops in the round's race week (`preview`, four days before the race) so it lands BEFORE the round it previews, never after (see `articleDate`). |
| `championship_state` | championshipArc | **Sparse, in-season (#88).** The drivers' title-fight narrative — fires only at trajectory inflections (a lead eroding/extending, a decider, a lead change), telling a comeback-on-merit apart from one handed over by leader DNFs. Covers the *story* of the fight; the precise clinch permutations are in `titleScenario`. |
...
| `preview_schedule` | previews | A run-up piece for **every round** (off the standings as they stood beforehand), plus the upcoming round while live. Beyond the title picture it carries: a **form** talking point (last race vs car-pace expectation — a standout to continue, OR an under-performer/DNF to bounce back, omitted if all ran to form); a **development** beat (the round's upgrade(s) and their projected order shift — from `devPlans` for rounds to come, the upgrade log for past ones — incl. a gap-closer when rank holds and a spokesperson quote when the part fails); and a **logistics** beat (lap count, the seeded weather forecast, a hedged pit-stop spread). All built from round-stable data, so a published preview never mutates. Replaced the per-race `technicalRoundup`. |
...
| `analysis_opinion` | bestOfRest | **End of season (#90).** The fight to lead the midfield — the best finisher among teams outside the preseason front tier, sharing the season review's definition so the two never disagree. A compressed band, a surge from a projected backmarker, or a clear win. |
...
| `legends` | legends (#93) | **"Remember this driver?", three a year on a fixed 4-month grid (any season phase).** One retired driver per slot, picked deterministically (seeded, never repeating across a save) from everyone OFFICIALLY retired — gone from the live roster (the 5-years-without-a-seat market rule), `lastRacedYear <= year - 5`. Each career is classified into ONE story angle (`great` / `nearly`-man / `loyal` midfielder / `outpaced` / `footnote`) and the piece ARGUES it as **lead → evidence → conclusion**, rather than stacking beats: a thesis with character up front, the supporting facts woven with transitions (the signature win framed as the charge it was, championship runner-ups, the champions who took the titles), and a conclusion that frames the all-time standing ("as of <month year>, Nth all-time in wins …", grouped by shared rank) with a wins/podiums-gated stature descriptor, tied back to the angle. Also-rans and footnotes get an honest one-line verdict instead. Facts are assembled SERVER-SIDE from the archive (`legends-data.ts`, `buildLegendData`) and passed on `ctx.legends`; the producer renders prose only, gendered pronouns resolved from history data + a live-captured `driver_genders` table, era-peers limited to title rivals (never teammates). Empty until a save has a driver gone five years. Carries an `absoluteDate` (its grid slot), bypassing the round-based `articleDate`. Interrupts by default. |
