...
/** Metres of lane per cull arc. A pit lane is a few hundred metres of road that a car crawls the length
 *  of, so the camera holds one part of it for a long time; cutting it at the same interval as the lap
 *  keeps the rest of it off the rasteriser while that happens. */
const ARC_M = 45
...
export interface PitSurface {
  ...
  ground: string
  /** Zoomed out the lane is a couple of pixels wide and a soft edge cannot be seen, so the softening
   *  layers collapse to one and the grain is dropped entirely. */
  detail?: 'full' | 'low'
}

const layersOf = (s: PitSurface) => (s.detail === 'low' ? 1 : SOFT_LAYERS)
...
/** One colour laid along a polyline in cullable arcs, `width` units wide. */
function strokeRuns(
  pts: readonly Vec[], runs: number[][], width: number, colour: string, pad: number,
): DrawOp[] {
  return runs.map((idx) => {
    const { d, clip } = stripe(pts, idx, () => 0)
    return { d, stroke: colour, width, cap: 'round', clip: { ...clip, r: clip.r + width / 2 + pad } }
  })
}
 ...
 *  wider than the road, cut into arcs that each carry a cull disc. The working apron was first done as
 *  its whole RING dilated by one stroke per layer, which is correct and five large fills spanning the
 *  entire box row -- on the one stretch of tarmac whose raster budget this branch has already measured
 *  itself out of. Stroking its outer EDGE instead paints the same fringe in arcs the viewport can skip,
 *  and the apron's interior needs no fade because the apron's own fill covers it.
 *
...
export function pitEdgeOps(s: PitSurface): DrawOp[] {
  ...
  for (let k = 0; k < layers; k++) {
    ...
    for (const e of edges) ops.push(...strokeRuns(e.pts, e.runs, 2 * u(e.halfM + reachM), colour, pad))
  }
  // The asphalt itself, over the fade and under the white line, so what the fade falls away FROM is
  // asphalt rather than paint.
  for (const e of edges) ops.push(...strokeRuns(e.pts, e.runs, 2 * u(e.halfM), s.tarmac, pad))
  return ops
}
...
function wearOps(s: PitSurface): DrawOp[] {
  ...
  for (let k = 0; k < layers; k++) {
    for (const idx of runs) {
      ...
      const op = softStroke(fast, idx, k, layers, { core, spread, colour, pad })
      if (op) ops.push(op)
    }
  }
  return ops
 ...
 *  the lane. Every box carries its own small cull disc, so driving the pit straight pays for the boxes
 *  in shot rather than for the whole row. */
function boxOps(s: PitSurface): DrawOp[] {
  const { u, boxes } = s
  if (!boxes || boxes.length === 0) return []
  const layers = Math.min(layersOf(s), BOX_LAYERS)
  const pad = u(0.6)
...
function bandOps(
  ...
  for (let band = 0; band < bands; band++) {
    ...
    for (const idx of chunk(allOf(n), perPatch)) {
      ...
      const { d, clip } = patch(quad)
      ops.push({ d, fill: blend(s.tarmac, w > 0 ? MARBLE : RUBBER, t), clip: { ...clip, r: clip.r + u(0.5) } })
    }
  }
  return ops
}

/** The lane's own grain. Skipped at zoom-out for the same reason the circuit's is: three grey levels on
 *  a road two pixels wide, at the price of several hundred ops on screen at once. */
function grainOps(s: PitSurface): DrawOp[] {
  const { u, fast } = s
  if (s.detail === 'low' || fast.length < 3) return []
