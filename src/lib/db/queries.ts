...
export function resetDatabase(): void {
  ...
  db.transaction(() => {
    ...
    db.prepare('DELETE FROM driver_genders').run()
...
export function insertDriverRaceAttributes(
  ...
  const db = getDb()
  const stmt = db.prepare(`
    INSERT INTO driver_race_attributes (season_id, round, driver_id, pace, wet_weather_pace, overtaking, smoothness, consistency)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    ON CONFLICT(season_id, round, driver_id) DO UPDATE SET
      pace = excluded.pace, wet_weather_pace = excluded.wet_weather_pace,
      overtaking = excluded.overtaking, smoothness = excluded.smoothness, consistency = excluded.consistency
  `)
  const insertMany = db.transaction((rows: DriverAttributeSnapshot[]) => {
    for (const s of rows) stmt.run(seasonId, round, s.driverId, s.pace, s.wetWeatherPace, s.overtaking, s.smoothness, s.consistency)
  })
...
export function getDriverRatingsHistory(driverId: string): DbRatingsPointRow[] {
  return getDb().prepare(`
    SELECT s.year AS year, dra.round AS round,
      ...
      dra.overtaking AS overtaking, dra.smoothness AS smoothness, dra.consistency AS consistency
...
export interface DbRaceResultRow extends DbRaceResult {
  ...
  circuit_id: string
  circuit_name: string
  form?: number | null   // pre-race form, joined for the driver-season path only
}
...
export function getDriverRacesInSeason(seasonId: number, driverId: string): DbRaceResultRow[] {
  return getDb().prepare(`
    ...
    LEFT JOIN driver_race_form drf ON drf.race_id = rr.race_id AND drf.driver_id = rr.driver_id
...
export interface DbTeamCareerAgg {
  teamId: string; races: number; seasons: number; wins: number; podiums: number; poles: number; points: number
}
...
export function getTeamConstructorRecordsUpToYear(year: number): DbTeamConstructorRecord[] {
  return getDb().prepare(`
    SELECT scs.team_id AS teamId,
      MIN(scs.final_position) AS bestFinish,
      SUM(CASE WHEN scs.final_position = 1 THEN 1 ELSE 0 END) AS titles
    FROM season_constructor_standings scs
    JOIN seasons s ON s.id = scs.season_id
    WHERE s.status = 'archived' AND s.year <= ?
    GROUP BY scs.team_id
  `).all(year) as DbTeamConstructorRecord[]
}
...
// --- All-time stats tables (standings page) — per driver / per team across ARCHIVED seasons. The
...
export function getAllTimeDriverStats(): AllTimeDriverStat[] {
  const rows = getDb().prepare(`
    SELECT rr.driver_id AS id, MAX(rr.driver_name) AS name,
...
export function getAllTimeTeamStats(): AllTimeTeamStat[] {
  const rows = getDb().prepare(`
    SELECT rr.team_id AS id, MAX(rr.team_name) AS name,
      COUNT(DISTINCT r.season_id) AS seasons,
      COUNT(DISTINCT r.id) AS races,
      MIN(s.year) AS firstYear, MAX(s.year) AS lastYear,
      SUM(CASE WHEN rr.finish_position = 1 THEN 1 ELSE 0 END) AS wins,
