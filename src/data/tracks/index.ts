...
// There used to be a `NIGHT_VENUES` set here, naming the six circuits that race under floodlights.
// It is gone: a venue is not night because it is on a list, it is night because of where it stands
// and what time it starts, and `data/tracks/venues.ts` already carries both. `isFloodlit` answers it
// from the sun's own altitude, which the list could not: Abu Dhabi starts in daylight and finishes
// in the dark, and no per-circuit boolean can say that.
